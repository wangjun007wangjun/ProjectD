...
namespace Engine.Http
{
    ...
    public class HttpFileSession
    {
        ...
        public void Begin()
        {
            ...
            {
                GGLog.LogE("HttpFileSession Url is Error");
                _isError = true;
                _isOver = true;
                return;
            }
            ...
            {
                GGLog.LogE("HttpFileSession DownloadFilePath is Error");
                _isError = true;
                _isOver = true;
                return;
            }
            _tempDownLoadFilePath = Engine.Res.FileUtil.CombinePath(HttpService.TempDir, Id.ToString() + ".temp");
            //
        ...
        private void OnAsyncRespond(IAsyncResult ar)
        {
                ...
                else
                {
                    ...
                    if (response != null)
                    {
                        if (response.StatusCode == HttpStatusCode.OK)
                        {
                            ...
                            //
                            if (Engine.Res.FileUtil.IsFileExist(_tempDownLoadFilePath))
                            {
                                Engine.Res.FileUtil.DeleteFile(_tempDownLoadFilePath);
                            }
                            //
                        ...
                        else
                        {
                            ...
                            {
                                GGLog.LogE("HttpFileSession--->Error:" + sc);
                                _isError = true;
                            }
                        }
                    }
            ...
            finally
            {
                ...
                {
                    GGLog.LogE("HttpFileSession Exception Or Error Finaly:" + Url);
                    _isError = true;
                }
            }
        }
        ...
        public void Update()
        {
            ...
            {
                GGLog.LogE("HttpFileSession Read Exception :" + ex.Message);
                isException = true;
            }
            finally
            {
                ...
                else
                {
                    if (readSize > 0)
                    {
                        ...
                        //JWLog.LogD("FileHttpSession:Downloaded:"+_curDownloadLength+":"+_totalFileLength);
                        if (Handler != null)
                        {
                        ...
                        if (_curDownloadLength >= _totalFileLength)
                        {
                            ...
                            if (_fileStream != null)
                            {
                                ...
                                _fileStream = null;
                                //移动文件
                                Engine.Res.FileUtil.CopyFile(_tempDownLoadFilePath, DownloadFilePath, true);
                                //删除临时文件
                                Engine.Res.FileUtil.DeleteFile(_tempDownLoadFilePath);
                            }
                        }
                    ...
                    {
                        GGLog.LogD("FileHttpSession:Downloaded End");
                        ////下载完成
                        if (_curDownloadLength >= _totalFileLength)
                        {
                            _isOver = true;
    ...
    public class HttpService: MonoSingleton<HttpService>, IScheduleHandler
    {
        ...
        //网络事务列表
        private readonly ArrayList<HttpFileSession> _httpSessions = new ArrayList<HttpFileSession>(3);
        ...
        void Start()
        {
            TempDir = Engine.Res.FileUtil.CombinePath(Engine.Res.FileUtil.GetCachePath(), "HttpTemp");
            if (Engine.Res.FileUtil.IsDirectoryExist(TempDir))
            {
                Engine.Res.FileUtil.ClearDirectory(TempDir);
            }
            else
            {
                Engine.Res.FileUtil.CreateDirectory(TempDir);
            }
        ...
        public uint RegisteFileHttpDownload(string url, string filePath, HttpFileSessionDelegate handler = null)
        {
            ...
            {
                GGLog.LogE("RegisteFileHttpDownload:Arg Error");
                return 0;
            }
