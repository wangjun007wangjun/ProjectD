...
namespace Engine.Schedule
{
    ...
    /// 调度服务
    /// </summary>
    public class ScheduleService : Singleton<ScheduleService>
    {
        /// <summary>
        /// 内部数据结构
        /// </summary>
        ...
        private readonly ArrayList<IScheduleHandler> _updates = new ArrayList<IScheduleHandler>(50);
        private readonly ArrayList<Data> _timers = new ArrayList<Data>(20);
        private readonly ArrayList<Data> _frames = new ArrayList<Data>(20);
        ...
        public void Update()
        {
            ...
            for (int i = 0; i < count && i < _updates.Count; i++)
            {
                ...
                if (handler != null)
                {
                    ...
                    {
                        GGLog.LogE("ScheduleService.Update : update exception {0} at handler {1}", e, handler.GetType().Name);
                        throw;
                    }
                }
                else
            ...
            for (int i = 0; i < count && i < _timers.Count; i++)
            {
                ...
                {
                    GGLog.LogE("ScheduleService.Update : timer exception {0} at handler {1}", e, handler.GetType().Name);
                    throw;
                }
            }
            ...
            for (int i = 0; i < count && i < _frames.Count; i++)
            {
                ...
                {
                    GGLog.LogE("ScheduleService.Update : frame exception {0} at handler {1}", e, handler.GetType().Name);
                    throw;
                }
            }
        ...
        private void Remove(ArrayList<Data> datas, uint id)
        {
        ...
        private void Remove(ArrayList<Data> datas, IScheduleHandler handler)
        {
            for (int i = 0; i < datas.Count; i++)
            {
