...
namespace Engine.NetAsset
{
    public class NetAssetInfoSet
    {
        ...
        private ObjList<NetAssetInfo> _cachedAssetInfos = new ObjList<NetAssetInfo>();
        private ObjDictionary<string, NetAssetInfo> _cachedAssetInfoMap = new ObjDictionary<string, NetAssetInfo>();
        ...
        public void RemoveNetAssetInfoAndDelFileByDays(float days)
        {
            for(int i = _cachedAssetInfos.Count-1; i >=0; i--)
            {
                ...
                {
                    //彻底清除
                    Engine.Res.FileUtil.DeleteFile(info.GetCachedFilePath());
                    _cachedAssetInfoMap.Remove(info.Key);
