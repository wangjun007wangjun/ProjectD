...
namespace Engine.Base
{
    ...
    public static class ExtTransform
    {
        ...
        public static void ExtSetPositionXY(this Transform tf, float x, float y)
        {
            if (tf == null)
            {
                return;
        ...
        public static void ExtRotateX(this Transform tf, float offX, bool isWorldSpace=false)
        {
            if (tf == null)
            {
                return;
            ...
            tf.Rotate(new Vector3(offX, 0, 0), isWorldSpace?Space.World:Space.Self);
        }
        public static void ExtRotateY(this Transform tf, float y, bool isWorldSpace=false)
        {
            if (tf == null)
            {
                return;
            ...
            tf.Rotate(new Vector3(0, y, 0), isWorldSpace?Space.World:Space.Self);
        }
        ...
        /// 设置物体的旋转方向
        /// </summary>
        /// <param name="transform">需要旋转的Transform</param>
        /// <param name="pos">目标坐标</param>
        public static void ExtSetRotation2D(this Transform transform, Vector3 pos)
        {
            if (transform == null)
            {
                return;
            }
            transform.right = (Vector2)pos - (Vector2)transform.position;
        }

        /// <summary>
        /// 设置旋转
        /// </summary>
        /// <param name="transform">需要旋转的Transform组件</param>
        /// <param name="dir">目标方向</param>
        public static void ExtSetRotationByDir2D(this Transform transform, Vector3 dir)
        {
            if (transform == null)
            {
                return;
            }
            transform.right = (Vector2)dir;
        }
        ...
        public static float ExtGetBetweenLength2D(this Transform tf, Transform other)
        {
            if(other == null)
            {
                return 0;
            }
            return Vector2.Distance(tf.position, other.position);
        }

       
        /// <summary>
        /// 将世界坐标转为对应Recttransform中的本地坐标
        /// </summary>
        /// <param name="worlTf">需要转的worldPosition</param>
        /// <param name="refTf">对应的rectransform</param>
        /// <param name="canvasTf">对应的canvas transform</param>
        /// <returns>返回相对于refTf的本地坐标位置</returns>
        public static Vector3 WorldToUILocalPoint(this Transform worlTf, Transform refTf, Camera camera)
        {   
            if(refTf == null)
            {
                return Vector3.zero;
            }
            RectTransform rectTf = refTf as RectTransform;
            if(rectTf == null)
            {
                return Vector3.zero;
            }
            ...
            RectTransformUtility.ScreenPointToLocalPointInRectangle(rectTf,camera.WorldToScreenPoint(worlTf.position), camera, out pos);
            return new Vector3(pos.x, pos.y, 0);
        }

        //附加到父节点
        public static void ExtAttachToParent(this Transform tf, Transform parent, bool active=true)
        {
            if (parent == null)
            {
                ExtSetNullParent(tf);
                ExtSetLocalPositionXYZ(tf, 0, 0, 0);
                ExtSetLocalEulerAnglesXYZ(tf, 0, 0, 0);
                ExtSetLocalScaleXYZ(tf, 1, 1, 1);
                return;
            }
            else
            {
                ExtSetParent(tf, parent);
                ExtSetLocalPositionXYZ(tf, 0, 0, 0);
                ExtSetLocalEulerAnglesXYZ(tf, 0, 0, 0);
                ExtSetLocalScaleXYZ(tf, 1, 1, 1);
            }
            ExtSetActive(tf, active);
        }
    }
