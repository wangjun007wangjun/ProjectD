...
namespace Engine.Res
{
    ...
    public class DeferUnloadData
    {
        ...
        static ObjList<DeferUnloadData> _pool = new ObjList<DeferUnloadData>();
    ...
    public class BundleService : MonoSingleton<BundleService>
    {
        ...
        /// <typeparam name="BundleRef"></typeparam>
        /// <returns></returns>
        ObjDictionary<string, BundleRef> _bundleDict = new ObjDictionary<string, BundleRef>(StringComparer.OrdinalIgnoreCase);
        ...
        /// </summary>
        /// <typeparam name="string"></typeparam>
        /// <typeparam name="DeferUnloadData"></typeparam>
        /// <returns></returns>
        Dictionary<string, DeferUnloadData> _unloadingBundle = new Dictionary<string, DeferUnloadData>();

        public ObjDictionary<string, BundleRef> BundleDict
        {
            get
            {
                return _bundleDict;
        ...
        public void LoadSync(BundlePackInfo packInfo)
        {
            if (packInfo == null)
            {
                GGLog.LogE("Sync loading bundle with null pack info.");
                return;
            }
            ...
            {
                GGLog.LogE("Bundle is loading when load sync. bundle:{0}", packInfo.Path);
                return;
            }
            ...
            GGLog.LogE("Sync load bundle path:{0}", path);
            //
        ...
        public IEnumerator LoadAsync(BundlePackInfo packInfo, BundleLoadedDelegate complete, BundleLoadFailedDelegate failure, BundleLoadingDelegate loading = null)
        {
            if (packInfo == null)
            {
                GGLog.LogE("Async loading bundle with null pack info.");
                yield break;
            }
...

            {
                ...
                string path = GetBundleFullPath(packInfo, false);
                GGLog.LogE("Async load bundle path:{0}", path);
                ...
                {
                    GGLog.LogE("Async loading bundle {0} failed, error:{1}", path, "");
                    if (failure != null)
                    {
                        failure(packInfo, "");
                    }
            ...
            /*Test2
            {
                ...
                JWLog.LogD("Async load bundle use UnityWebRequest path:{0}", path);
                ...
                if (ab == null)
                {
                    JWLog.LogE("Async loading bundle use UnityWebRequest {0} failed, error:{1}", path, "");
                    if (failure != null)
                    {
                        failure(packInfo, "");
                    }
        ...
        /// <param name="loading">加载进度回调</param>
        /// <returns></returns>
        public IEnumerator LoadAsync(ObjList<BundlePackInfo> packInfos,
            ...
            BundleLoadingDelegate loading = null)
        {
            ...
            for (int i = 0; i < packInfos.Count; ++i)
            {
                ...
                if (packInfo != null)
                {
                    ...
                    // load
                    string path = GetBundleFullPath(packInfo, true);
                    GGLog.LogE("Async load bundle liat path:{0}", path);
                    ...
                    if (!string.IsNullOrEmpty(www.error))
                    {
                        GGLog.LogE("Async loading bundle {0} failed, error:{1}", packInfo.Path, www.error);
        ...
        /// <returns></returns>
        public IEnumerator BatchLoadAsync(ObjList<BundlePackInfo> bundleList, Action complete, BundleBatchLoadingDelegate progress = null, BundleLoadedDelegate loaded = null, BundleLoadFailedDelegate failure = null)
        {
            ...
            // 并发加载多个bundle
            ObjList<string> failedBundles = null;
            ...
            for (int i = 0; i < bundleList.Count; i++)
            {
                ...
                }, delegate (BundlePackInfo packInfo, string error)
                {
                    ...
                    {
                        failedBundles = new ObjList<string>();
                    }
        ...
        public IEnumerator BatchLoadAsync(ObjList<BundlePackInfo>[] bundleLists, Action complete, BundleBatchLoadingDelegate progress = null)
        {
            ...
            {
                ObjList<BundlePackInfo> bundles = bundleLists[i];
                if (bundles != null)
                {
            ...
            for (int i = 0; i < bundleLists.Length; ++i)
            {
                ...
                }, delegate (BundlePackInfo packInfo, float prog)
                {
                    ...
                    if (progress != null)
                    {
                        ...
                        {
                            ObjList<BundlePackInfo> bundlePackInfos = bundleLists[bundleListIndex];
        ...
        public void OnAssetLoaded(BundlePackInfo pi)
        {
            ...
            if (pi.Life == EBundleLife.Immediate)
            {
                ...
                {
                    GGLog.LogE("Immediately unload bundle when it's already in unloading queue. bundle:{0}", pi.Path);
                }
            }
        }
        ...
        /// 真正卸载无用bundle
        /// </summary>
        public void UnloadUnusedBundles(ObjList<string> willUseAssets = null)
        {
            ObjList<string> unloaded = null;
        ...
        public void Unload(BundlePackInfo pi, bool immediately = false)
        {
            ...
            if (_bundleDict.TryGetValue(pi.Path, out br))
            {
                if (pi.Life == EBundleLife.Cache)
                {
                    ...
                    {
                        GGLog.LogE("Unloading bundle [{0}] while its reference count is zero.", pi.Path);
                    }
                ...
                else if (pi.Life == EBundleLife.Resident)
                {
                    ...
                    {
                        //ToDo OneJun
                        //JWLog.LogE("Unloading RESIDENT bundle {0}", br.Path);
                    }
                }
            }
            else
            {
                GGLog.LogE("Unloading bundle which is not exist. path:{0}", pi.Path);
            }
        }
