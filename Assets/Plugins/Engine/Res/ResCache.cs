...
namespace Engine.Res
{
    ...
    {
        ObjDictionary<string, ResObj> _resources;

        public ResCache()
        {
            _resources = new ObjDictionary<string, ResObj>(StringComparer.OrdinalIgnoreCase);
        }
        ...
        /// <returns>返回true表示所有同路径资源都已经移除完</returns>
        public bool Remove(ResObj resource, bool immediately)
        {
            ...
            if (_resources.ContainsKey(resource.OriginPath))
            {
                ...
                else
                {
                    ...
                    {
                        GGLog.LogE("Unloading resource [{0}] while its reference count is zero.", resource.Path);
                    }
        ...
        /// 卸载资源
        /// </summary>
        /// <param name="willUseAssets"></param>
        public void UnloadUnusedResources(ObjList<string> willUseAssets = null)
        {
