...
namespace Engine.Res
{
    ...
    {
        bool Android_IsFileExistInStreamingAssets(string fileName);
        byte[] Android_ReadFileInAssets(string fileName);
    }
    ...
    public static class FileUtil
    {
        ...
        private static string _unityCachePath = null;

        //IFS解压路径 游戏首次运行会先对安装包的资源first.zip 解压到外面 此目录对应editor 的Resources打包资源外围叫IFS 
        ...
        //代理
        public static FileUtilInterface Proxy;
        ...
        public static bool IsFileExistInStreamingAssets(string fileName)
        {
            if (Application.platform == RuntimePlatform.Android)
            {
                if (Proxy != null)
                {
                   return Proxy.Android_IsFileExistInStreamingAssets(fileName);
                }
                else
                {
                    GGLog.LogE("ResUtil Android Need Proxy");
                    return false;
                }
            }
            else
        ...
        public static bool CreateDirectory(string directory)
        {
            ...
            while (true)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    if (tryCount >= 3)
                    {
                        GGLog.LogE("Create Directory " + directory + " Error! Exception = " + ex.ToString());
                        //派发事件
        ...
        public static bool DeleteDirectory(string directory)
        {
            ...
            while (true)
            {
                ...
                catch (System.Exception ex)
                {
                    ...
                    if (tryCount >= 3)
                    {
                        GGLog.LogE("Delete Directory " + directory + " Error! Exception = " + ex.ToString());
                        //派发事件
        ...
        public static int GetFileLength(string filePath)
        {
            ...
            while (true)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    if (tryCount >= 3)
                    {
                        GGLog.LogE("Get FileLength of " + filePath + " Error! Exception = " + ex.ToString());
                        return 0;
                    }
                }
            }
        ...
        public static byte[] ReadFile(string filePath)
        {
            ...
            while (true)
            {
                ...
                {
                    GGLog.LogE("Read File " + filePath + " Error! Exception = " + ex.ToString() + ", TryCount = " + tryCount);
                    data = null;
                }
                ...
                if (data == null || data.Length <= 0)
                {
                    ...
                    if (tryCount >= 3)
                    {
                        GGLog.LogE("Read File " + filePath + " Fail!, TryCount = " + tryCount);
                        //派发事件
        ...
        public static byte[] ReadFileInStreamingAssets(string fileName)
        {
            ...
            if (Application.platform == RuntimePlatform.Android)
            {
                if (Proxy!=null)
                {
                    data = Proxy.Android_ReadFileInAssets(fileName);
                }
                else
                {
                    GGLog.LogE("ReadFileInStreamingAssets Android Need Proxy");
                }
            }
            else
            {
        ...
        public static bool WriteFile(string filePath, byte[] data, bool createDirectory = false)
        {
            ...
            while (true)
            {
                ...
                catch (System.Exception ex)
                {
                    ...
                    if (tryCount >= 3)
                    {
                        GGLog.LogE("Write File " + filePath + " Error! Exception = " + ex.ToString());
                        //删除文件以防止数据错误
                        DeleteFile(filePath);
                        //派发事件
        ...
        public static bool DeleteFile(string filePath)
        {
            ...
            while (true)
            {
                ...
                catch (System.Exception ex)
                {
                    ...
                    if (tryCount >= 3)
                    {
                        GGLog.LogE("Delete File " + filePath + " Error! Exception = " + ex.ToString());
        ...
        public static string GetFileMd5ByFileStream(string filePath)
        {
            ...
            {
                GGLog.LogE("GetFileMd5 Exception"+e.ToString());
                return "";
            }
        }
        ...
        public static string GetStreamingAssetsPath(string fileName = "")
        {
            ...
            {
                return CombinePaths(Application.dataPath, "!/assets/", fileName);
            }
        }
        ...
        public static string GetStreamingAssetsPathWithHeader(string fileName)
        {
            ...
            {
                return CombinePaths(Application.streamingAssetsPath, fileName);
            }
            return "";
        }
        ...
        {
            if (_unityCachePath == null)
            {
                if (Application.platform == RuntimePlatform.Android)
                {
                    _unityCachePath = CombinePath(GetCachePath(), "UnityCache");
                }
                else
                {
                    _unityCachePath = string.Format("{0}/../UnityCache", GetCachePath());
                }
            }
            GGLog.LogD("GetUnityCachePath : {0}", _unityCachePath);
            return _unityCachePath;
        }
        ...
        public static string GetCachePath()
        {
            if (_cachePath == null)
            {
                ...
                {
                    _cachePath = Application.dataPath + "/../Cache/";
                }
                else
                {
                    if (Application.platform == RuntimePlatform.Android)
                    ...
                    {
                        _cachePath = Application.streamingAssetsPath + "/../../Cache/";
                    }
                }
            }
            return _cachePath;
