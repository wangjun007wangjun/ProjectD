...
namespace JWEngine.Event
{
    public class EventRouter : Singleton<EventRouter>
    {
        public ObjDictionary<string, Delegate> m_eventTable = new ObjDictionary<string, Delegate>();
        ...
        public void BroadCastEvent(string eventType)
        {
            if (OnBroadCasting(eventType))
            {
                ...
                {
                    GGLog.LogE("There is no handler registered for event [" + eventType + "]!");
                }
            }
        }
        ...
        private bool OnHandlerRemoving(string eventType, Delegate handler)
        {
            ...
            if (m_eventTable.ContainsKey(eventType))
            {
                ...
                if (d != null)
                {
                    ...
                    {
                        GGLog.LogE("Failed to remove handler of type [" + handler.GetType().Name + "] for event [" + eventType + "] which is expected handler of type [" + d.GetType().Name + "]");
                        result = false;
                    }
                }
                else
                {
                    GGLog.LogE("Failed to remove handler of type [" + handler.GetType().Name + "] which was not found!");
                    result = false;
                }
            }
            else
            {
                GGLog.LogE("Failed to remove handler of type [ " + handler.GetType().Name + "], event [" + eventType + "] was not found!");
                result = false;
            }
