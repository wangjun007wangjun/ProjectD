...
namespace UnityEditor.XCodeEditor
{
	public partial class XCProject : System.IDisposable
	{
		...
		public XCProject( string filePath ) : this()
		{
			...
			Debug.Log ("project.pbxproj file contents: " + contents);
		...
		public bool BitCodeEnableExceptions(string value)
		{
			foreach( KeyValuePair<string, XCBuildConfiguration> buildConfig in buildConfigurations ) {
				buildConfig.Value.BitCodeEnableException( value );
			}
			modified = true;
			return modified;
		}
		...
		public PBXDictionary AddFile( string filePath, PBXGroup parent = null, string tree = "SOURCE_ROOT", bool createBuildFiles = true, bool weak = false )
		{
			...
			Debug.Log ("fileReference.buildPhase: " + fileReference.buildPhase);
		...
		{	
			...
			foreach( XCModFile libRef in mod.libs ) {
				...
				completeLibPath = completeLibPath.Replace (@"\", "/");
			...
			foreach( string headerpath in mod.headerpaths ) {
				if (headerpath.Contains("$(inherited)")) {
					...
					absoluteHeaderPath = absoluteHeaderPath.Replace (@"\", "/");
